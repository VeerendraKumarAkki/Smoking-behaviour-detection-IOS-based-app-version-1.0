...
class MainTableViewController: UITableViewController, ScanTableViewControllerDelegate {
    ...
    var intOperation:Int?
    
    
    override func viewDidLoad() {
        super.viewDidLoad()
        //print("activity in view load\(activityNumber) ")
        if let activityToDisplay = activityNumber {
            
            print("activity:\(activityToDisplay)")
            
        }
        if let mode = intOperation {
            
            print("mode in Main view controler 1 for data collection 0 for User Experiment  :\(mode)")
    ...
    //    print("inside mainview controller prepare\(intOperation)")
    //   //let destination = segue.destination as? ScanTableViewController
    //  if segue.identifier == "ViewDevice"{
    //     print("inside destination \(intOperation)")
    //    //var destination1 = segue.destination as! UINavigationController
    //   let target = segue.destination as! DeviceViewController
    //  target.operation = intOperation
    //}
